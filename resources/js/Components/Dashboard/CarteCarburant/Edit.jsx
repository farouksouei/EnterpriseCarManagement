...
export default function EditCarteCarburant({ close, vehicles,model }) {
    console.log("model," , model)
    const { data, setData, put, reset, errors } = useForm({
        litres: model.litres,
        max_litres: model.max_litres,
        plafond: model.plafond,
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        put(route('carte-carburants.update', model.id), {
            data,
            onSuccess: () => {
                reset();
                close();
